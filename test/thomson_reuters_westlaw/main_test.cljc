(ns thomson_reuters_westlaw.main-test
  "Contract + behavioral test for the thomson_reuters_westlaw-compat L4 actor (cljc port).
  ...
  (:require [clojure.test :refer [deftest is testing run-tests]]
            ...
            [thomson_reuters_westlaw.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "thomson_reuters_westlaw-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'thomson_reuters_westlaw.main-test)]
