(ns thomson_reuters_westlaw.main
  "Kotodama WASM entrypoint for the Thomson_reuters_westlaw clean-room actor (L4) — Clojure port.
  ...
  facts (`thomson_reuters_westlaw.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "thomson_reuters_westlaw")
(def tier "L4")
...
(def entity-specs
  [{:entity "Matter"          :plural "matters"         :id-prefix "thomsonr_mat"
    :fields [:clientId :name :status :openedAt]                :required [:name :status]
    :coerce {}                                    :refs {}}
   {:entity "Document"        :plural "documents"       :id-prefix "thomsonr_doc"
...
(defn emit-facts
  "EAVT facts for one record: {\"thomson_reuters_westlaw.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "thomson_reuters_westlaw-compat" :tier tier :entities entities} 200])
...
(defn start! [] :thomson_reuters_westlaw-compat/ready)
